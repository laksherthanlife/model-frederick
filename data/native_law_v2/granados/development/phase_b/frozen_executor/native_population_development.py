...
def predict_development(prepared, fits, protocol):
    require(tuple(fits) == MODEL_IDS, "all six canonical families must remain present")
    prefix = {row["group_id"]: row["mu"] for row in prepared["prefix_summaries"]}
    predictions, audit = [], []
...
def prepare_projections(protocol, training, conditioning, training_sha, conditioning_sha):
    ...
    prefixes, training_frames, coordinates, audits = [], [], [], []
...
def write_implementation_configuration(root, protocol_path, output_directory):
    ...
    configuration = {"document_type": "population_development_executor_configuration", "protocol_id": protocol["protocol_id"], "protocol_sha256": protocol_ref["sha256"], "code_artifacts": implementation, "code_snapshots": snapshots, "environment": environment_record(), "requested_phase": "before_training_value_export", "strong_claim_authorized": False}
...
def write_release_request(root, protocol_path, output_directory, exporter_paths, requester):
    root = Path(root).resolve()
    protocol = scoped_protocol(root, protocol_path)
    ...
    if existing.is_file():
        ...
        require(configuration["code_artifacts"] == implementation and configuration["environment"] == environment_record() and configuration["protocol_sha256"] == current_protocol["sha256"] == snapshot["sha256"], "existing pre-value implementation freeze differs from the current request")
    else:
        frozen = write_implementation_configuration(root, protocol_path, output_directory)
...
def _admission(root, protocol_path, allowlist_path, phase, freeze_sha=None):
    root = Path(root).resolve()
    protocol = scoped_protocol(root, protocol_path)
    protocol_ref = artifact_reference(root, protocol_path, "protocol_snapshot")
...
def check_custody_gate(root, protocol_path, allowlist_path=None, phase="training_and_conditioning_export", freeze_sha=None):
    try:
        protocol, allowlist, provenance = _admission(root, protocol_path, allowlist_path, phase, freeze_sha)
        return {"ready": True, "scientific_scope": SCIENTIFIC_SCOPE, "measurement_arrays_opened": False, "approved_artifacts": allowlist["artifacts"], "provenance": provenance, "strong_claim_authorized": False}
...
def execute_training(root, protocol_path, allowlist_path, output_directory):
    protocol, allowlist, provenance = _admission(root, protocol_path, allowlist_path, "training_and_conditioning_export")
    store = ArtifactStore(root, output_directory)
